...
random.seed()
#print "go8b run number " + str(runNumber) + " has random seed: " + str(rndseed)

file1 = open('/scratch/fy9/go8b/lst_reactions_go8b.pkl', 'rb')
lst_reactions = pickle.load(file1)
file1.close()

file2 = open('/scratch/fy9/go8b/lst_metabolites_go8b.pkl', 'rb')
lst_metabolites = pickle.load(file2)
file2.close()

file3 = open('/scratch/fy9/go8b/edges_go8b.pkl', 'rb')
...
print "go8b run number " + str(runNumber) + " has maximum modularity: " + str(maxM)
# this will show up in the .out files in /scratch/fy9/run folder

# Print community strcuture
filename = '/scratch/fy9/go8b/commStruc_go8b_run' + runNumber + '.pkl'
...
for key, value in s.items():
    for node in value:	
        partition[node]=key
filename = '/scratch/fy9/go8b/partition_go8b_run' + runNumber + '.pkl'
file = open(filename, 'wb')
...
filename = '/scratch/fy9/go8b/cocluster_go8b_run' + runNumber + '.pkl'
file = open(filename, 'wb')
