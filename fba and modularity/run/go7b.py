...
random.seed()
#print "go7b run number " + str(runNumber) + " has random seed: " + str(rndseed)

file1 = open('/scratch/fy9/go7b/lst_reactions_go7b.pkl', 'rb')
lst_reactions = pickle.load(file1)
file1.close()

file2 = open('/scratch/fy9/go7b/lst_metabolites_go7b.pkl', 'rb')
lst_metabolites = pickle.load(file2)
file2.close()

file3 = open('/scratch/fy9/go7b/edges_go7b.pkl', 'rb')
...
print "go7b run number " + str(runNumber) + " has maximum modularity: " + str(maxM)
# this will show up in the .out files in /scratch/fy9/run folder

# Print community strcuture
filename = '/scratch/fy9/go7b/commStruc_go7b_run' + runNumber + '.pkl'
...
for key, value in s.items():
    for node in value:	
        partition[node]=key
filename = '/scratch/fy9/go7b/partition_go7b_run' + runNumber + '.pkl'
file = open(filename, 'wb')
...
filename = '/scratch/fy9/go7b/cocluster_go7b_run' + runNumber + '.pkl'
file = open(filename, 'wb')
